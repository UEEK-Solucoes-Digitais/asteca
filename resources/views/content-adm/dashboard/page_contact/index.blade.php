        ...
        @section("content")
            @php $instance_name = "Pagina de contato" @endphp
            @csrf
            <div class="default-space-between">
                ...
                <div class="d-flex">
                    <a class="btn-geral btn-green" href="{{ route("page_contact.add") }}">
                ...
                <div class="table-responsive">
                    <table class="table nowrap" id="dataTable" data-order='[[0, "desc"]]' width="100%" cellspacing="0">
                        ...
                        <tbody class="tdbody-to-sortable">
                            ...
                            @foreach ($page_contact as $page_contact)
                                <tr id="item-{{ $page_contact->id }}">
                                    <td>
                                        <div class="option-table-area">
                                            <input type="checkbox" name="delete-itens[]" class="multiple-delete"
                                                value="{{ $page_contact->id }}">
                                            <div class="checkbox-area">
                                                <iconify-icon icon="bi:check"></iconify-icon>
                                            </div>
                                        </div>
                                    </td>
                                    <td><b>{{ $page_contact->id }}<b></td>
                                    <td>{{ $page_contact->title }}</td>
                                    <td>{{ date("d/m/Y H:i:s", strtotime($page_contact->created_at)) }}</td>
                                    <td>
                                        <div class="d-flex">
                                            <a href="{{ route("page_contact.edit", ["id" => $page_contact->id]) }}"
                                                class="btn-yellow btn-action" title="Editar {{ $instance_name }}">
                                                {!! $pen_iconify !!}
                                            </a>
                                            <button class="btn-red btn-action remove-item" data-value="{{ $page_contact->id }}"
            ...
            <script>
                var url_organize = "{{ route('page_contact.organize') }}";
                var url_delete = "{{ route('page_contact.delete') }}";
                var url_delete_multiple = "{{ route('page_contact.delete_multiple') }}";
                var url_to_redirect = "{{ route('page_contact.list') }}"
            </script>
        @endsection
